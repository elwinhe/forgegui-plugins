...
- **Trace the client's motion, not only the outcome.** Acceptance metrics driven by AI throughput (hits, goals, pickups, "moved less than 2 studs per frame") passed while the player's own car skipped every frame: the controller wrote the chassis `CFrame` 63–66 times in three seconds to settle it. A per-frame client trace (that trace was run by a separate pass; the metric it beat is ours) (moving frames, zero-motion frames, peak step residual, max frame time) caught it; after the fix the same trace showed 0 direct writes and a 0.29-stud peak step. Record the trace beside the screenshots, and have a person drive it.
...
- Refracting `Glass` in front of a `SurfaceGui` hides its text. Use transparent `SmoothPlastic` (or another non-refracting material) for a shell that has readable surfaces behind it.
