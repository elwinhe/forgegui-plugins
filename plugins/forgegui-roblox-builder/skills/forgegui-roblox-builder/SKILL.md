...
- **Verify it by running it, on the client.** A script that compiles in `run_code` or `execute_luau` has been checked at plugin security, not at client security; a separate agent's pass on the same game reported a `LocalScript` reading `SurfaceAppearance.ColorMap` raising a Plugin capability error after the plugin-side write had succeeded, which we did not reproduce. Play the place and read the client's own output. Two faults in the September 19, 2026 showcase only surfaced this way: `StreamingEnabled` kept server-made vehicles from ever reaching the client, and `StarterGui` never loaded because the place spawned no avatars.
- **"Code-verified" is not verified.** Two rounds of the same build reported particle effects as working from the code alone; the first capture in Play showed the boost flame and speed streak had never emitted, because `Enabled` was rewritten every frame. Capture the effect while it is supposed to be running, or say it was not seen.
- **Trace the client's motion, not only the outcome.** Acceptance metrics driven by AI throughput (hits, goals, pickups, "moved less than 2 studs per frame") passed while the player's own car skipped every frame: the controller wrote the chassis `CFrame` 63–66 times in three seconds to settle it. A per-frame client trace (moving frames, zero-motion frames, peak step residual, max frame time) caught it; after the fix the same trace showed 0 direct writes and a 0.29-stud peak step. Record the trace beside the screenshots, and have a person drive it. The trace itself was run by a separate pass; the throughput metric it exposed was ours.
- **Keep the failed probes.** A single missed contact or a stalled frame in a probe log is a reproduction lead, not noise to remove from the report. Reproduce it in isolation and record both runs; a metric that passes only after its outliers were deleted proves nothing.
...
- Non-colliding parts can still intercept clicks and raycasts. Check `CanQuery` and decorative descendants, not just `CanCollide`: decorative trim with `CanQuery = true` tilted a raycast-suspended car in the September 19, 2026 showcase. Set `CanQuery` and `CanTouch` off on pure decoration.
- Studio's viewport redraws only while its window is frontmost. A `screen_capture` or window capture of a background Studio is the last frame it drew, so an agent can "verify" a stale image. Bring Studio to the front before every capture and put a changing marker (a clock label, a frame counter) in shot when a capture loop has to prove liveness.
- Animate joints through `Motor6D.Transform`, not by rewriting `C0`/`C1` every frame; the bind pose stays fixed and the update belongs in `PreSimulation` with the physics step. Repeated `C0` writes on the wheel rig were one source of the per-frame skipping above (reported by a separate pass on the same build; not reproduced here).
- Refracting `Glass` in front of a `SurfaceGui` hid its text in the one case observed; a transparent non-refracting material such as `SmoothPlastic` fixed it. Inferred from before-and-after captures rather than a controlled comparison.
- Read a generated mesh's axes and descendants before fitting it: one generated body had its long axis on X and its nose on -X, so a yaw of -π/2 was needed; the bounds alone do not say which end is the front. Imported models can also carry scripts; check before parenting.
- When something is missing from the client view, separate the causes before fixing any of them: streaming (`StreamingEnabled`, radius), asset fetch (`PreloadAsync`, moderation), graphics quality level, atmosphere or fog distance, and geometry that was never built. A captured before/after pair with the same camera settles which one it was.
