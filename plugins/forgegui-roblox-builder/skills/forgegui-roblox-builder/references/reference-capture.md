...
A request that carries a likeness claim ("like X", "same style as", "like the real thing") is a claim you will be measured against. A supplied reference is required to make or measure a likeness claim. If the user declines or is unavailable, proceed with a style-only build, record `"reference": { "supplied": false }` in the ledger, make no likeness claim, and report no likeness percentage. A fidelity pass without an external target measures the model against its own memory: it converges on a remembered game rather than the real one, and reports a likeness figure nothing outside the model can check.
...
3. **If the user declines or is not available to ask,** build to the style words in the request. Record it in `forgegui-project.json` as `"reference": { "supplied": false }`, make no likeness claim, and do not report a likeness percentage anywhere in the report.
...
- **Palette:** `scripts/palette.py refs/<selected-capture>/ --k 6` prints hex values with proportions from supported images, excluding `sheet_*` contact sheets. Sample the play surface and each team or faction colour separately if they matter; record the hex values, not colour names.
