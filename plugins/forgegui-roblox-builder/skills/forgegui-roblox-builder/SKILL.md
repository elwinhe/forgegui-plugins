...
- Animate joints through `Motor6D.Transform`, not by rewriting `C0`/`C1` every frame; the bind pose stays fixed and the update belongs in `PreSimulation` with the physics step. Repeated `C0` writes on the wheel rig were one source of the per-frame skipping above.
