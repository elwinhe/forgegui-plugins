...
- **Animation.** Verified September 19, 2026 (`evidence/spike-animation` in the showcase repo): an agent-authored R15 `KeyframeSequence` built in `run_code`, serialized with `SerializationService:SerializeInstancesAsync`, base64-encoded with `EncodingService:Base64Encode` (`HttpService` has no such method), rebuilt as `.rbxm` locally and uploaded through the same personal-key Open Cloud route as PR #1 with `assetType: "Animation"` returned an id that was `Approved` at once and played on a standard R15 character in Play (`Animator:LoadAnimation`, `IsPlaying = true`, arm rotated 140.7°). No hosted bridge, no auto-rigging and no Studio dialog were involved. A GLB or FBX is not accepted as an Animation; convert motion to a `KeyframeSequence` first. Playback was checked only in a place owned by the uploading account.
- **Clothing.** An uploaded `assetType: "Image"` id works directly as `Shirt.ShirtTemplate` / `Pants.PantsTemplate` on an R15 character (`evidence/spike-shirt`): `PreloadAsync` succeeded and the texture wrapped the torso and limbs. Open Cloud cannot upload Shirt or Pants assets and does not need to for in-experience clothing; do not plan a Marketplace upload for it.
...
6. For dependent 3D work pass the owner-scoped `source_job_id` the schema requires. Never forward a provider task id. Rig only compatible characters; animate only from a completed rig. Motion that must play on a standard R15 avatar goes in as a `KeyframeSequence` uploaded as `Animation` (§4), not as a rigged GLB.
