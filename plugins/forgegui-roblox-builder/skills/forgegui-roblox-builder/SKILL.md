...
- **Skybox.** Generate one 2:1 panorama and cut it into the six `Sky` faces with `scripts/sky_faces.py`; do not generate six faces separately, because their edges will not meet. Roblox's face mapping is not the obvious one (measured in Studio September 19, 2026: `SkyboxLf` shows on +X, `SkyboxUp` turned 90° clockwise, `SkyboxDn` 90° counter-clockwise) and the script already undoes it; `--selftest` pins that table, but only the in-Studio captures show it is the mapping Studio needs. Apply through `references/luau/SkyboxPresets.luau`, which replaces every `Sky` under `Lighting` — stacked `Sky` instances are why a new one may not be the one rendered — and otherwise shares `LightingPresets.luau`'s ownership convention; run the lighting preset first and the skybox preset second. See `references/skybox.md`.
