...
- **An image reference replaces the content of a 3D call, not just its style.** Passing a scene concept's `mcp-artifact:` ref to `generation_model_3d` returned the whole concept scene as one mesh (a car and a ball fused; an island, tower, bridge and tree fused) in 13 of 13 calls across two styles on September 19, 2026, whatever the prompt asked for. Give a 3D call only an image of that one object, or no image at all and carry the style in the prompt. `generation_image` and `generation_gui` take the same concept ref without hijacking, so scene concepts belong to 2D calls.
...
4. On `outcome_unknown`, retain identifiers and check status later. Do not regenerate while the state can still resolve. Use `generation_retry` only when the job state permits it and the budget allows. `outcome_unknown` can also be terminal: 3 of the 32 first-pass Turbo Pitch jobs on September 19, 2026 ended `outcome_unknown` with `retryable: false`, and status never moved. Once status confirms that terminal state, a replacement under a new `request_id` is not a duplicate; record the link between the two entries in the ledger. An `outcome_unknown` returned within a second of queueing, on several jobs in a row, was the 3D face of the account's generation limit (below), not a provider fault; stop and check the limit before submitting more.
5. **The account has a generation ceiling with no headroom signal.** After roughly sixty successful calls in one day (September 19, 2026, `evidence/generation-quota` in the showcase repo) every image, GUI and 3D call failed with `"Generation rate limit exceeded"`, `retryable: false`, for at least an hour; 17 spaced retries over 56 minutes all failed, and the limit cleared on its own between 57 and 109 minutes after the first refusal. A shorter burst limit hit earlier: two of three image retries were refused after about ten images in thirty minutes (`turbo-pitch/assets/INDEX.md`). Nothing in the tool output warns that either ceiling is near, so plan the day's count up front, generate the assets that unblock other work first (concepts, the skybox, hero models), and spread calls out rather than bursting. Do not retry a `retryable: false` refusal in a tight loop; wait and probe with one call.
6. Stop paid actions on insufficient credits, missing scope, or entitlement rejection. Report the returned failure; never attempt a bypass. Never infer a refund from an error.
7. For dependent 3D work pass the owner-scoped `source_job_id` the schema requires. Never forward a provider task id. Rig only compatible characters; animate only from a completed rig.
8. Update the ledger entry written in step 1 with the finished artifact: `job_id`, `artifact_ref`, kind, prompt summary, and later the Roblox asset id or instance path.
