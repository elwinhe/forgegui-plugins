...
- **Style identities, when the live server exposes them.** Discover `style_list`, `style_get`, `style_create` and the generation schema before use. Resolve one identity per project before the first paid visual generation: validate an existing pin with `style_get`, or list candidates and read their briefs before reusing or creating an identity from the user's style answer plus the manifest's `art_direction` and `material_language`. Map returned `id`/`version` to manifest `style_id`/`style_version` and pass **both together** on `generation_model_3d` only when its live schema supports them. `style_brief` and image-backed `style_reference_asset_ids` are per-call additions valid only alongside that pin; object plus resolved style references, including saved images, cannot exceed 6. Style management calls charge no credits. Full procedure, accepted image UUID/artifact references, upload route, and revision rules: `references/style-identity.md`. When the tools are absent, use the manifest's text and compatible image references without unsupported style fields — do not hunt for missing tools or report their absence as an error.
