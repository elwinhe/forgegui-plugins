...
Install the shared sibling `LightingCompositor` even when using GI alone. For GI plus ColourGrade, use [the combined setup](unified-lighting.md).
...
| --- | --- | --- | --- | --- |
| GI level (`setLevel`) | ≤ 0.01: GI colour layer removed immediately, bounce lights fade out | < 0.75: 10-stud cells, ±4 cells, 24 bounce lights, 24 cells traced a frame | 6-stud cells, ±7 cells, 64 lights, 48 cells a frame | the ground probe runs every 0.25 s at both levels: 28 down rays and 16 up rays |
...
- `getSunColour`: defaults to the compositor's ungraded base `ColorShift_Top`, avoiding grade feedback into the bounce probe.
...
The conventions are the same as in `LightingPresets.luau`. Only attribute-tagged instances are ever destroyed, never matched by name, and borrowed properties are recorded as `ForgeGUIPrior_<Property>` on the instance they belong to, except GI colours, which use the shared LightingCompositor base.
...
GI and ColourGrade submit layers to one shared LightingCompositor. Only it writes their four Lighting colour properties. It tracks actual engine readback to distinguish external base changes from its own quantized output, and always composes base → GI → grade. GI eases probe colour/openness, not previously composed Lighting output.

LightingPresets and SkyboxPresets retain independent prior attributes. Their baseline capture uses the unlayered compositor base when the current value is its output; neither GI nor ColourGrade claims or clears their priors. Clear skybox before lighting preset. Either runtime layer can stop first.

Occasional external writes become the base on the next update or stop. For continuous day/night animation, pass GI `getBase` and stop directly writing those properties; the callback supplies un-bounced, ungraded colours, including its latest value on stop. Use `LightingCompositor.setBase` when GI is absent or for explicit writes equal to the current visible output.
...
1. Run the regression in an owned test place. Install the modules with their shared compositor dependency as described in `docs/evidence/light-transport-regression.luau` as its header shows, run from a LocalScript, and expect `PASS … checks passed`. It checks ownership, restore and idempotence for every module, and puts back everything it touched.
