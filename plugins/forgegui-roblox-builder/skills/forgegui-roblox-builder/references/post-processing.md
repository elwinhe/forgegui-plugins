...
| `Lighting.Ambient`, `OutdoorAmbient`, `ColorShift_Top` | LightingPresets / your day-night script, then ColourGrade | ColourGrade multiplies its split tone into whatever value someone else last wrote; priors `ForgeGUIColourGradePrior_*` |
...
- **Priors survive composition.** If LightingPresets (or SkyboxPresets) applies while a layer is on, it records the layered value as its `ForgeGUIPrior_*`. The layer notices the external write and rewrites that record to the value underneath, so `LightingPresets.clear()` still returns the original place.
...
- **Stale sessions.** `stop()` on a module that never started (a fresh `require` from `execute_luau`, or after a crash) still sweeps its own attributes and owned instances back.

`LightingPresets.luau` is unchanged. Its presets set `depthOfField` (mostly `Enabled = false`); with CameraFx running the DoF is driven every frame, so the preset's DoF values only return when CameraFx stops. `dungeon_torchlit` wants a resting DoF on: CameraFx's resting DoF replaces it.
