...
- **Check mesh topology before you upload, not after a player sees it.** `scripts/glb_quality.py <file.glb>` reports triangles, vertices, **boundary edges** (an edge with a triangle on one side only, so the surface stops there), non-manifold edges, and duplicate or degenerate triangles; `--selftest` covers a closed and an open shape. A hole *through* a shape is not the same thing: a wheel with an open hub is still a closed surface and reports zero, while a cup, a pipe or a flat banner is genuinely open and reports more — correctly. The count is a question, not a verdict: where does this mesh stop, and should it? A generated mesh can stop somewhere wrong without looking it, because the thumbnail renders a closed shape either way, remeshing reduces the count without necessarily closing the surface, and neither upload nor moderation inspects topology. Roblox renders a single side, so an opening the player sees into shows nothing at all — in one build a hat's crown was missing and the head showed through. Read the count, look at the mesh from the angles the player gets, and remesh, regenerate or cap only what should have been solid.
