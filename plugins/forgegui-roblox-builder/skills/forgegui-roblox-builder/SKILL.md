...
- **Audio, including ForgeGUI's own.** A generated clip reaches Roblox by the ordinary upload route, so the sound layer is not limited to the audio library. Measured September 20, 2026: `generation_sound_effect` (and `generation_music`) returns a terminal `succeeded` job whose result carries `audio_urls` and an `artifact_ref` of kind `audio` — a downloadable `.mp3`. Fetch that file and upload it as `assetType: "Audio"`; the returned id works as `Sound.SoundId` like any other. One 2-second effect (job `b948de0a`, 33 KB, 128 kbps 44.1 kHz) uploaded this way and returned an id while moderation still read `Reviewing`. Generation and import are separate questions: the generator produces a file, and the same Open Cloud route that takes an image or a model takes that file. What is genuinely constrained is *playback*: every uploaded audio asset is private to its creator by default, so it plays in a place opened by the uploading account and raises "User is not authorized to access Asset" anywhere else. Before handing a place to someone else, grant their experience `Use` on the asset (`PATCH /asset-permissions-api/v1/assets/permissions`, scope `asset-permissions:write`) or re-upload under their credentials, and check `IsPublicDomain` when you expect an id to work for everyone.
