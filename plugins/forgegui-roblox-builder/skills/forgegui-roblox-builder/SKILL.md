...
7. Update the ledger entry written in step 1 with `job_id`, delivery mode, kind and prompt summary. For direct delivery, record the available `artifact_ref`; for published delivery, record `publication_id`, decimal-string `asset_id`, `asset_type`, creator metadata (`creator_group_id`) and delivery status as returned, without requiring an artifact ref. Add the Studio instance path and verification after insertion; keep publication readiness separate from generation and Studio success.
