...
- **Check mesh topology before uploading a mesh.** `scripts/glb_quality.py <file.glb>` reports **boundary edges** — an edge with a triangle on one side only, so the surface stops there — plus non-manifold, duplicate and degenerate triangles; `--selftest` covers hand-built shapes and a round trip through a real GLB. A hole *through* a shape is not an open surface: a wheel with an open hub reports zero, while a cup, a pipe or a banner is open by design and correctly reports more. The count earns its keep on a surface that stops where it should not, which a thumbnail will not show: one hat in a September 2026 build reported 3 boundary edges, about one missing triangle, and Roblox renders a single side, so the player looked down and saw through the crown.
